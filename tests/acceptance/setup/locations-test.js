...
let application;
let locationsUrl = `/gridiron/${orgId}/admin/setup/locations`;
...
test('Locations page basic UI, with no existing locations', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.locations', 'remains on locations step');
...
test('Locations page requires a location, unless remote only is indicated', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.locations', 'remain on current location');
  ...
  andThen(() => {
    ...
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.team', 'moved to next page');
  });
});
...
test('Clicking back should return you to previous step', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.organization', 'returned to organization step');
  });
});
...
test('Adding an incomplete location shows an error message', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.locations', 'remains on location step');
    fillInLocation();
  });
...
test('Clicking continue creates locations attestation', function(assert) {
  ...
  andThen(clickContinueButton);
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.team', 'on next setup step');
  });
});
