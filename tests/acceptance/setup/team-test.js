...
test('You are redirected to correct step if not ready for team step', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.organization', 'redirected to organization step');
  });
});
...
test('Clicking back should return you to previous step', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'requires-authorization.gridiron.gridiron-organization.gridiron-admin.gridiron-setup.locations', 'returned to locations step');
  });
});
