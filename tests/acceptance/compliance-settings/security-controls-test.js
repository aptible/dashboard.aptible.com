...
test('Basic UI', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'compliance.compliance-organization.engines.compliance-settings.security-controls.show', 'on show page');
  });
});
...
test('Resuming with existing attestations', function(assert) {
  ...
  andThen(() => {
    assert.equal(currentPath(), 'compliance.compliance-organization.engines.compliance-settings.security-controls.show');
